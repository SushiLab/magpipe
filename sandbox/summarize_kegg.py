...
"""
This script parses the annotations output to summarise all KEGG annotations 
table
"""
...
def main(annotation_dir, output_table):
    ...
    Reads the list of genomes and process the kegg tables to generate a concatenated annotation file 
    """
    ...
    table_columns = ["gene", "KO", "COG", "EC", "Description"]
    ...
    with open(output_path, 'w') as target:
        ...
        for genome_path in tqdm(annotation_path.glob('*'), ncols=100):
            ...
            kegg_file = genome_path.joinpath(f"kegg/{genome}-kegg-annotations.tsv")
